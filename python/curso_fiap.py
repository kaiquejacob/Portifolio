...
contados_suportados = ("telefone", "email", "endereco")

#Dicionario de exemplo,com alguns dados padrão
agenda = {
    "Pessoa 1": {
...
def contato_para_texto(nome_contato:str, **formas_contato):
    """Recebe um nome de contato com string e um dicionário com formas de contato.
    Retorna um string com os dados recebidos"""
...
#Função de vizualização da agenda completa
def agenda_para_texto(**agenda_completa):
    """Recebe um dicionário de dicionários com a agenda que será exibida e retorna um string com este dicionário formatado"""
...
def altera_nome_contato(agenda_original:dict, nome_original:str, nome_atualizado:str):
    """Recebe a agenda original em forma de dicionário , o nome_original e o nome_atalizado em forma de string.
...
def exclui_contato(agenda:dict, nome_contato:str):
    ...
    Caso o nome do contato não esteja nas chaves do dicionário, retorna False
    Caso o nome do contato esteja nas chaves,o refistro correspondente será removido e retornara True"""
...
def inclui_contato(agenda:dict, nome_contato:str, **formas_contato):
    """Recebe uma agenda completa para dicionário, o nome do novo contato como string e as formas de contato
    em um dicionário como **kwargs.
    Não é feita nenhuma verificação, portanto se já existir um contato com o mesmo nome , será sobrescrito"""
...
def inclui_forma_contato(formas_contato:dict, forma_incluida:str, valor_incluido:str):
    """Recebe um dicionário com formas de contato, a forma de contato que será incluida ou
    alterada e o valor que será incluido.
    Caso a forma de contato já possua valores, o novo valor será adicionado na lista e retornara True.
    ...
    Caso a forma decontato ainda não exista e não estiver presentena tupla de formas de contatos suportados,
    retornará False"""
    if forma_incluida in formas_contato.keys():
        formas_contato[forma_incluida].append(valor_incluido)
        return True
    elif forma_incluida in contados_suportados:
...
def usuario_inclui_contato(agenda:dict):
    ...
    dicionario_formas = {}
    for forma in contados_suportados:
...
def usuario_inclui_forma_contato(agenda:dict):
    ...
    if nome in agenda.keys():
        print(f"As formas de contato suportados pelo sistema são: {contados_suportados}")
        forma_incluida = input("Qual forma de contato deseja incluir: ")
        if forma_incluida in contados_suportados:
            ...
            if inclui_forma_contato(agenda[nome], forma_incluida, valor_incluido):
                ...
                print("Ocorreu um erro durante a inserção. A agenda foi alterada.")
        else:
            print("A forma de contato indicada não é suportada pelo sistema. A agenda não foi alterada.")
    else:
        print("Ocontato informado não existe na agenda. Não foram feitas alterações.")

def usuario_exclui_contato(agenda:dict):
    nome = input("Informe o nome do conatto que deseja exluir: ")
    if exclui_contato(agenda, nome):
        print("Usuário excluido com sucesso!")
    else:
        print("Nome de usuário não localizado na agenda. Não foram feitas alterações.")
...
def usuario_altera_nome_contato(agenda:dict):
    ...
    nome_original = input("Informe o nome do conatto que deseja alterar: ")
    nome_atualizado = input("Informe o novo nome do contato: ")
...
def usuario_altera_forma_contato(agenda:dict):
    ...
    if nome in agenda.keys():
        print(f"As formas de contato suportadas pelo sistema são: {contados_suportados}")
        forma_incluida = input("Qual forma de contato deseja incluir: ")
        if forma_incluida in contados_suportados:
    ...
    while op != 11:
        ...
        if op == 1:
           ...
           usuario_contato_para_texto(agenda)
            ...
            nome_arquivo = input("Informe o novo ou cominho do arquivo: ")
